...
export const install: ViteModule = async ({ isClient, app }: any) => {
  if (isClient) {
    const L = await import("leaflet");

    app.config.globalProperties.$VT = L.GridLayer.extend({
      options: {
        async: true,
      },
      ...
      createTile: function (coords) {
        ...
        var tile = L.DomUtil.create("canvas", "leaflet-tile-custom");
        tile.style.pointerEvents = "initial";
        tile.style.zIndex = "850";
      ...
      drawFeature: function (ctx, feature) {
        ...
        if (type === 3) ctx.fill("evenodd" || this.options.style.fillRule);

        ctx.stroke();
      },

      setStyle: function (ctx, style) {
        var stroke = true || style.stroke;
        if (stroke) {
          ctx.lineWidth = 5 || style.weight;
    ...
    L.geoJson.vt = async function (geojson, options) {
