...
<template>
  <div class="home-page">
    <div class="modal-buttons">
      <button class="burger-button" @click="onBurgerButtonClick()">
        <span class="iconify" data-icon="mdi:menu" data-width="24" data-height="24" />
      </button>

      <button class="modal-button" v-if="menuShowing" @click="showAboutIMSModal">
        <span
          class="iconify"
      ...
      <InfoModal v-if="aboutIMSModalShowing" title="Sobre o IMS">
        <template #default>
          <TextBodySmall>
            O Sistema de Gestão Georreferenciada Integrada é uma plataforma baseada no
            CITinova, apoiada pelo GEF, que está sendo desenvolvida pelo NGPD e ARIES.
          </TextBodySmall>
        </template>
      ...
      <ProjectListModal v-if="projectListModalShowing" :projects="projects" />
    </div>
  </div>
</template>
